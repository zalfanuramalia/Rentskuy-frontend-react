...
export const Home = ({getPopularHome}) => {
  const {popular: populars, auth} = useSelector (state => state);
      ...
      <Layout>
        ...
        <footer className='container-md-fluid control'>
          ...
          {auth.userData.role === 'Admin' ? (
            <div className='d-flex justify-content-center align-items-center mx-5 px-5'>
              <div className='button-chat btn btn-primary col-12'>
                <Link to='/add-vehicles' style={{textDecoration:'none'}} type="button"> <p className='mt-3'>Add Item</p></Link>
              </div>
            </div>
          ) : (
            ''
